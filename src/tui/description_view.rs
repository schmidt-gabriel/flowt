use crate::config::{TriggerConfig, NodeKind};
use crate::tui::App;
use ratatui::{
...
impl App {
    pub fn draw_description_view(&self, f: &mut Frame) {
        ...
        let (workflow_name, description, triggers_info) = if !workflows.is_empty() && self.selected_workflow < workflows.len() {
            ...
            let nodes_info = if nodes_count == 0 {
                ...
                format!("{} node{}", nodes_count, if nodes_count == 1 { "" } else { "s" })
            };
            ...
            ("No workflow selected".to_string(), "Please select a workflow first.".to_string(), "".to_string())
        };
        ...
        let mut description_lines = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(workflow_name, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
        ...
        if !workflows.is_empty() && self.selected_workflow < workflows.len() {
            ...
            description_lines.push(Line::from(vec![
                Span::styled("Nodes:", Style::default().fg(Color::DarkGray).add_modifier(Modifier::BOLD)),
            ]));
            description_lines.push(Line::from(""));

            if workflow.nodes.is_empty() {
                description_lines.push(Line::from(vec![
                    Span::styled("  No nodes configured", Style::default().fg(Color::DarkGray)),
                ]));
            } else {
                for (index, node) in workflow.nodes.iter().enumerate() {
                    // Node header with ID and type
                    description_lines.push(Line::from(vec![
                        Span::styled(format!("  {}. ", index + 1), Style::default().fg(Color::Yellow)),
                    ...
                    match &node.kind {
                        NodeKind::Http { url, method, expect_status, .. } => {
                            ...
                            if let Some(status) = expect_status {
                                description_lines.push(Line::from(vec![
                                    Span::styled("     Expect: ", Style::default().fg(Color::DarkGray)),
                                    Span::styled(format!("Status {}", status), Style::default().fg(Color::Yellow)),
                                ]));
                            }
                        }
                        NodeKind::Shell { cmd, env } => {
                            ...
                            if !env.is_empty() {
                                description_lines.push(Line::from(vec![
                                    ...
                                    Span::styled(
                                        env.keys().map(|k| k.as_str()).collect::<Vec<_>>().join(", "),
                    ...
                    if !node.depends_on.is_empty() {
                        description_lines.push(Line::from(vec![
                            ...
                            Span::styled(node.depends_on.join(", "), Style::default().fg(Color::Red)),
                        ]));
                    }
                    ...
                    if !optional_props.is_empty() {
                        description_lines.push(Line::from(vec![
                            ...
                            Span::styled(optional_props.join(", "), Style::default().fg(Color::Cyan)),
                        ]));
                    }
